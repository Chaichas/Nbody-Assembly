...
//						      accelerations[i] = accelerations[i]+(GravConstant * masses[j] / (pow(sqrt((positions.x[i]-positions.x[j])*(positions.x[i]-positions.x[j])+(positions.x[i]-positions.x[j])*(positions.y[i]-positions.y[j])), 3) + 1e7))*(position.x[j]-position.x[i]);
    }
}
